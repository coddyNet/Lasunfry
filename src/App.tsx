...
export function App() {
  ...
  // Route -> State Sync
  useEffect(() => {
    if (!user) return; // Only sync URLs to state if logged in
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      const remainingIds = openFileIds.filter(openId => openId !== confirmDeleteId);
      setOpenFileIds(remainingIds);
      
      if (activeFileId === confirmDeleteId) {
  ...
  return (
    <Routes>
      ...
      <Route path="/file/:fileId" element={workspaceScreen} />
